...
#编译服务器和客户端
cserver="g++ .\server.c -o .\project\server -lwsock32"
cclient="g++ .\client.c -o .\project\client -lwsock32"
os.system(cserver)
os.system(cclient)
            ...
            if(connectnum<maxnum):
                ...
                for i in range(len(usednum)): #判断该序列号是否使用过
                    if(str(usednum[i])==str(num)):
                        ...
                        break
                if(find==0):
                    print("您输入的序列号有误！无法连接软件")
                else:
                    create_client=".\project\server" #用户运行软件，向许可证服务器发送验证
